...
// Base storage keys - will be prefixed with quizId
const BASE_STORAGE_KEY = "medical-intake-form-data";
const BASE_STEP_STORAGE_KEY = "medical-intake-form-step";

// Helper to get quiz-specific storage key
const getStorageKey = (quizId: string, baseKey: string): string => {
  return `${baseKey}_quiz_${quizId}`;
};
...
export function useFormPersistence(quizId?: string) {
  ...
  const clearLocalStorage = () => {
    ...
    try {
      if (quizId) {
        // Clear quiz-specific storage
        localStorage.removeItem(getStorageKey(quizId, BASE_STORAGE_KEY));
        localStorage.removeItem(getStorageKey(quizId, BASE_STEP_STORAGE_KEY));
      } else {
        // Clear default storage (for backward compatibility)
        localStorage.removeItem(BASE_STORAGE_KEY);
        localStorage.removeItem(BASE_STEP_STORAGE_KEY);
      }
    } catch (error) {
      // Ignore clear errors
    }
  ...
  const saveFormData = (formAnswers: FormAnswers) => {
    if (!quizId) {
      console.warn("No quizId provided to useFormPersistence, using default storage");
    }
    // Create a sanitized version that excludes large file data
    const sanitizedData = sanitizeFormDataForStorage(formAnswers);
    const storageKey = quizId ? getStorageKey(quizId, BASE_STORAGE_KEY) : BASE_STORAGE_KEY;
    saveToLocalStorage(sanitizedData, storageKey);
  };
  ...
  const saveCurrentStep = (stepIndex: number) => {
    ...
    const storageKey = quizId ? getStorageKey(quizId, BASE_STEP_STORAGE_KEY) : BASE_STEP_STORAGE_KEY;
    saveToLocalStorage(stepIndex, storageKey);
  };
  ...
  const loadLastStep = (): number => {
    ...
    const storageKey = quizId ? getStorageKey(quizId, BASE_STEP_STORAGE_KEY) : BASE_STEP_STORAGE_KEY;
    const step = loadFromLocalStorage(storageKey);
