<template>
  <div class="mx-auto bg-backgroundColor min-h-[98vh] flex flex-col items-center">
    <DynamicSection
    ...
    <div v-motion :initial="{ opacity: 0, y: 32 }" :visible-once="{
      ...
      <form @submit.prevent="handleSubmit" class="flex flex-col gap-6 px-6">
        <div>
          <label class="block text-lg md:text-xl text-black mb-1">{{ contactCopy.labels.name }}</label>
          <input v-model="formData.name" :placeholder="contactCopy.placeholders.name" type="text" :class="[
        ...
        <div>
          <label class="block text-lg md:text-xl text-black mb-1">{{ contactCopy.labels.age }}</label>
          <input v-model="formData.age" :placeholder="contactCopy.placeholders.age" type="number" min="1" max="120" :class="[
            'w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-accentColor1',
        ...
        <div>
          <label class="block text-lg md:text-xl text-black mb-1">{{ contactCopy.labels.email }}</label>
          <input v-model="formData.email" :placeholder="contactCopy.placeholders.email" type="email" :class="[
            'w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-accentColor1',
        ...
        <div>
          <label class="block text-lg md:text-xl text-black mb-1">{{ contactCopy.labels.message }}</label>
          <textarea v-model="formData.message" :placeholder="contactCopy.placeholders.message" rows="4" :class="[
            'w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-accentColor1',
...
// Load pages/sections so ContactHero can read from pages.json + sections.json
await pagesStore.loadAll().catch((err) => {
  console.error('Failed to load pages/sections data for Contact page:', err)
})

// Localized contact form copy (no dependency on external JSON content)
const contactCopy = {
  labels: {
    name: 'Name',
    age: 'Age',
...
// Sections for the contact page (e.g., Contact Hero)
const pageSections = computed(() => {
  return pagesStore.getPageSections('contact')
})
...
const handleSubmit = async () => {
  ...
  try {
    ...
    alert(contactCopy.successMessage)

  } catch (error) {
    console.error('❌ Form submission error:', error)
    alert(contactCopy.errorMessage)
  } finally {
