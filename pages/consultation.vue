...
import { getQuizById, glp1WeightLossQuiz } from "~/data/intake-form/quizConfigs";
import { intakeFormConfig } from "~/data/intake-form/config";
import { products as staticProducts, getProductById } from "~/data/intake-form/products";
...
onMounted(async () => {
  try {
    ...
    // Use quizId from product, or fallback to default quiz
    const finalQuizId = quizId || intakeFormConfig.defaultQuizId;
    
    // Load the quiz configuration
    const loadedQuiz = getQuizById(finalQuizId);
    
    if (!loadedQuiz) {
      quizError.value = `Quiz configuration not found for ID: ${finalQuizId}`;
      // Fallback to default quiz
      quizConfig.value = glp1WeightLossQuiz;
    } else {
      quizConfig.value = loadedQuiz;
    }
  } catch (error) {
