<template>
  <section v-if="content?.show" class="w-full flex justify-center">
    <div class="w-full flex flex-col py-20 pt-0 md:py-24 px-6 lg:px-0 max-w-[1056px]">
      ...
      <!-- Title (configurable from JSON) -->
      <div class="mt-10">
        <h3 class="text-[20px] md:text-[32px] font-semibold">
          {{ content?.title || 'Reach your goal weight' }}
          <span v-if="content?.highlight" class="text-accentColor1">{{ content.highlight }}</span>
        </h3>
      </div>

      <!-- Form fields (hardcoded - not configurable) -->
      <div v-for="(field, index) in formFields" :key="index" class="mt-6 md:pt-p flex flex-col gap-2">
...
// Hardcoded form fields (not configurable)
const formFields = [
  {
    label: 'Name',
